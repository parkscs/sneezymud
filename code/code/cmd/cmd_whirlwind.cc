...
int TBeing::doWhirlwind()
{
  ...
  TThing *primaryHandObj = this->heldInPrimHand();
  TBaseWeapon *primaryHandWeapon = dynamic_cast<TBaseWeapon *> (primaryHandObj);

  if (!primaryHandObj || !primaryHandWeapon){
      this->sendTo("You need to hold a weapon in your primary attack to attempt this maneuver.\n\r");	 
      return FALSE;
  }
  ...
  if (primaryHandWeapon->isBluntWeapon())
    damageType = DAMAGE_CAVED_SKULL;
  else if (primaryHandWeapon->isPierceWeapon())
    damageType = DAMAGE_IMPALE;
  else if (primaryHandWeapon->isSlashWeapon())
