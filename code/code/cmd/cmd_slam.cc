...
static int slam(TBeing *caster, TBeing *victim)
{
  ...
  if (!victim->awake() || 
    (successfulHit && successfulSkill && successfulHit != GUARANTEED_FAILURE)) {
      ...
      for (auto iterator = scalingDamageConstants.begin(); iterator != scalingDamageConstants.end();++iterator) {
        if (victim->GetMaxLevel() <= iterator->first) {
          scalingConstant = iterator->second;
          break;
        }
      }
